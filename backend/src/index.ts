...
const git_llave = process.env.OPENAI_API_KEY;
...
//promt para traducit
const generateTranslationPrompt = (textToTranslate: string) => {
    return ` Tu tienes un rol de traductor y requiero traduzcas al italiano este texto: ${textToTranslate}`
}


//promt para traducit muchos
const generateTranslationMPrompt = (textToTranslate: string, idioma: string) => {
    return ` Tu tienes un rol de traductor y requiero que traduzcas al ${idioma} este texto: "${textToTranslate}"`
}
...
//intento de traductor
app.post('/translate', async (req, res) => {
    const {prompt} = req.body
    const completion = await openai.createCompletion({
        model: 'gpt-3.5-turbo-instruct',
        prompt: generateTranslationPrompt(prompt),
        temperature: 0.1
    })
    // @ts-ignore
    res.send({result: completion.data.choices[0].text.trim(), token: completion.data.usage.total_tokens})
